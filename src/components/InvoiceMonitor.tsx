...
export default function InvoiceMonitor() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    for (const m of merged) {
      ...
      if (currentStage === 'cancelled' || currentStage === 'complete') continue;
  ...
  async function setStage(job: JobCard, stage: string) {
    ...
    if (error) { toast.error('Failed to update job status'); setProcessingId(null); return; }
    if (stage === 'cancelled' && job.assignment_id) {
  ...
  return (
    <div className="page-shell">
      ...
      <main className="page-content space-y-4">
        ...
        {/* Kanban board */}
        {!loading && (
          <div className={`pb-4 ${filterStage === 'all' ? 'overflow-x-auto' : ''}`}>
            <div className={`flex gap-3 ${filterStage === 'all' ? 'min-w-max' : ''}`}>
              {(filterStage === 'all' ? STAGES : STAGES.filter(stage => stage.key === filterStage)).map(stage => {
