...
const PRE_BOOKABLE_TYPES: ServiceType[] = ['Pool Opening 1st Visit', 'Pool Closing'];

const SEASONAL_SERVICE_TYPES: ServiceType[] = ['Pool Opening 1st Visit', 'Pool Closing'];
...
function DayViewModal({ date, onClose }: { date: string; onClose(): void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const [{ data: assignmentData }, { data: pendingBookings }] = await Promise.all([
        supabase.from('team_daily_assignments').select('title,service_type,display_address,status').eq('assignment_date', date).order('sort_order'),
        supabase.from('bookings').select('client_name,email,service_type,status').eq('event_date', date).eq('status', 'pending'),
      ]);
      const pendingEmails = (pendingBookings || []).map((booking: { email: string }) => booking.email).filter(Boolean);
      const { data: clients } = pendingEmails.length
        ? await supabase.from('clients').select('email,address,city,zip').in('email', pendingEmails)
        : { data: [] };
      const clientMap = new Map((clients || []).map((client: { email: string; address: string | null; city: string | null; zip: string | null }) => [client.email.toLowerCase(), client]));
      const assignmentRows: DayAssignment[] = (assignmentData || []).map((row: { title: string; service_type: string; display_address: string | null; status: string | null }) => {
        const parts = (row.display_address || '').split(',').map((part: string) => part.trim());
        return { ...row, city: parts.length >= 2 ? parts[parts.length - 2] : parts[0] || '', status: row.status || 'Confirmed' };
      });
      const pendingRows: DayAssignment[] = (pendingBookings || []).map((booking: { client_name: string | null; email: string; service_type: string }) => {
        const client = clientMap.get(booking.email.toLowerCase());
        const displayAddress = client ? [client.address, client.city, client.zip].filter(Boolean).join(', ') : null;
        return { title: booking.client_name || booking.email, service_type: booking.service_type, display_address: displayAddress, city: client?.city || '', status: 'Pending' };
      });
      setRows([...assignmentRows, ...pendingRows]);
      setLoading(false);
    })();
  }, [date]);
                  ...
                  <div key={i} className="flex items-center gap-3 px-5 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {r.status === 'Pending' && <span className="text-xs text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full font-medium">Pending</span>}
                        {r.city && (
                          <span className="text-xs text-neutral-500 flex items-center gap-1">
                            <MapPin className="w-3 h-3" />{r.city}
                          </span>
                        )}
                        {r.display_address && <span className="text-xs text-neutral-500 flex items-center gap-1 basis-full"><MapPin className="w-3 h-3 shrink-0" />{r.display_address}</span>}
                      </div>
                    </div>
                  </div>
                ))}
...
export default function BookClient() {
  ...
  const isSeasonalService = watchedServiceType ? SEASONAL_SERVICE_TYPES.includes(watchedServiceType) : false;
    ...
    try {
      ...
      if (mode === 'pre_book') {
        ...
        const bookingPayload: Record<string, unknown> = {
          ...
          closing_add_ons: clientRow?.pool_closing_add_ons || '',
          status: 'approved', job_status: 'booked', requested_by: technician.staff_id || technician.id || '',
        ...
        <div className="card card-body">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              {isSeasonalService ? (
                <div>
                  <label className="form-label">{isPreBook ? 'Preferred Date (optional)' : 'Select Date'}</label>
                  <p className="text-xs text-neutral-500 mb-2">Click a date to see scheduled cities and booked clients. The selected date becomes your booking date.</p>
                  <BookingCalendar
                    service={watchedServiceType as 'Pool Closing' | 'Pool Opening 1st Visit'}
                    selectedDate={watchedDate || null}
                    onSelectDate={(d) => setValue('eventDate', d, { shouldValidate: true })}
                  />
                  {errors.eventDate && <p className="form-error mt-2">{errors.eventDate.message}</p>}
                  {isPreBook && <p className="form-hint mt-2">Leave blank if no preferred date — admin will choose.</p>}
                </div>
              ) : (
                <div>
