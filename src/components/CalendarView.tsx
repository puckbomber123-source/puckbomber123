...
export default function CalendarView() {
  ...
  async function handleDeleteAssignment(id: string) {
    if (!window.confirm('Delete this assignment? The linked booking will be cancelled.')) return;
    const { data: linked } = await supabase.from('bookings').select('id').eq('assignment_id', id).maybeSingle();
    const { error } = await supabase.from('team_daily_assignments').delete().eq('id', id);
    if (error) { toast.error('Delete failed'); return; }
    if (linked?.id) {
      await supabase.from('bookings').update({
        job_status: 'cancelled', assignment_id: null, updated_at: new Date().toISOString(),
      }).eq('id', linked.id);
    }
    toast.success('Assignment deleted — linked booking cancelled');
    fetchRange();
  }
  ...
  function renderMonthGrid() {
    ...
    return (
      <div className="flex-1 overflow-auto">
        <div className="grid grid-cols-7 border-l border-t border-neutral-200 min-w-[460px]">
          {WEEKDAYS.map(d => (
  ...
  function renderWeekView() {
    ...
    return (
      <div className="flex-1 overflow-auto">
        <div className="grid grid-cols-7 border-l border-t border-neutral-200 min-w-[560px]">
          {days.map((day, i) => {
            const ds = formatDate(day);
            const dayAssignments = getAssignmentsForDate(ds);
            const today = isToday(day);
  ...
  return (
    <div className="min-h-screen bg-neutral-100 flex flex-col">
      <header className="bg-white border-b border-neutral-200 px-3 sm:px-6 py-2.5 sm:py-3 flex items-center justify-between gap-2 sticky top-0 z-20 shadow-card">
        ...
        <div className="flex bg-neutral-100 rounded-lg p-0.5 text-sm shrink-0">
          {(['month', 'week', 'day'] as ViewMode[]).map(m => (
            <button key={m} onClick={() => setViewMode(m)}
              className={`px-2.5 sm:px-3 py-1.5 rounded-md capitalize font-medium transition ${viewMode === m ? 'bg-white text-brand-600 shadow-card-md' : 'text-neutral-600 hover:text-neutral-800'}`}>
              {m}
            </button>
          ))}
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <button onClick={() => setCurrentDate(new Date())} className="btn-secondary btn-sm hidden sm:inline-flex">Today</button>
          <button onClick={() => navDate(-1)} className="btn-icon"><ChevronLeft className="w-4 h-4" /></button>
          <span className="text-xs sm:text-sm font-semibold text-neutral-700 min-w-[100px] sm:min-w-[140px] text-center">{headerLabel()}</span>
